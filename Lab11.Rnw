...
<<echo = F, message = T, warning = F>>=
data = read_csv("lab11data.csv")

data = data |>
  mutate(difference = Closer.vals - Farther.vals)
# a 
ttest.closer = t.test(data$Closer.vals, mu = 0)
ttest.closer

# b
ttest.farther = t.test(data$Farther.vals, mu = 0)
ttest.farther

# c
ttest.difference = t.test(data$difference, mu = 0)
ttest.difference
@
